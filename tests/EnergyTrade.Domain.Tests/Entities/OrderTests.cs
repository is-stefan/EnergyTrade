...
public class OrderTests
{
    ...
    public void Constructor_WithValidData_CreatesOpenOrder()
    {
        ...
        Assert.Equal(EnergyType.Solar, order.EnergyType);
        Assert.Equal(100m, order.QuantityMWh);
        Assert.Equal(85m, order.MaxPricePerMWh);
        Assert.Equal(Currency.EUR, order.Currency);
        Assert.Equal(deliveryStart, order.DeliveryStart);
        Assert.Equal(deliveryEnd, order.DeliveryEnd);
    ...
    [Fact]
    public void Cancel_WhenOrderIsAlreadyFilled_ThrowsInvalidOperationException()
    {
        var order = CreateValidOrder();
        order.Fill();
